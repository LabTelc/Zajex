# -*- coding: utf-8 -*-
"""
Created on Wed Aug 02 09:47:13 2017

@author: Vopalensky
"""

import FlatPanelFunctions as fp
import matplotlib.pyplot as plt
import time
import numpy as np
import _thread 
#from ctypes import c_ushort
#import soloistPluginVop as sp

#reload(fp)

def GetDeviceCnt():
    return fp.gb_if_get_device_cnt()[1]

def GetDeviceList(deviceCnt):
    return fp.gb_if_get_device_list(deviceCnt)[1]
...
class Detector():
    ...
    def Init(self):
        ...
        #print "v Initiu"
        (status, self.hAcqDesc) = fp.gb_if_init(cAddress = self.params.ucIP)
        #print "po GbIF Init"
        if status == 0:
            #print "pred input buf"
            self.inputBuf = fp.define_dest_buffers(self.hAcqDesc, frames= 1)[1]         #there will be no ring buffer, but just single-frame buffer
    ...
    def SetBinningMode(self, wMode):
        ...
        self.inputBuf = fp.define_dest_buffers(self.hAcqDesc, frames= 1, rows= self.nRows, columns= self.nColumns)[1]         #there will be no ring buffer, but just single-frame buffer
...
#            
    ...
    def createBuffer(self, npArray, type):
    ...
    def redraw(self, f, data):
        self.im.set_data(data)
        self.im.set_clim(np.nanpercentile(data,1), np.nanpercentile(data, 99))
        plt.title("Image No %d" %(f))
        plt.pause(0.1)
